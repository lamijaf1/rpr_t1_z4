...
public class Program {
    ...
    public static void DodajPredmet(Predmet p) throws IllegalArgumentException{
        boolean ima = false;
        for (int i = 0; i < predmeti.length; i++) if (predmeti[i].getSifraPredmeta() == p.getSifraPredmeta()) ima = true;
        if(ima==true)throw new IllegalArgumentException("greska");
        if(!ima) {
   ...
   public  static void removeElementStudent() {
    ...
    UnosPredmeta();
    DodajPredmet(new Predmet("rpr", 1010, 22));
    boolean x=true;
    while(x==true){
